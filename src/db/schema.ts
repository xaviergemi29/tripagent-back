...
export const PASSENGER_STATUS = pgEnum("passenger_status", [
    "ACTIVE",
    "CANCELLED"
]);
...
export const PASSENGER_TYPE = pgEnum("passenger_type", [
    "ADULT",
    "CHILD"
]);

export const AGENCY_ROLES = pgEnum("agency_roles", [
    "ADMIN",
    "SALES",
    "GUIDE"
]);
...
export const tours = pgTable("tours", {
    ...
    transportModality: TOUR_MODALITIES("transport_modality")
    ...
    boardingPoints: jsonb("boarding_points").$type<{ id: string; time: string; location: string }[]>().default([]).notNull(),
...
export const bookings = pgTable("bookings", {
    ...
    travelerId: uuid("traveler_id").references(() => travelers.id, { onDelete: "cascade" }).notNull(),
    ...
    amountPaid: numeric("amount_paid", { precision: 10, scale: 2, mode: "number" }).default(0).notNull(),
...
export const bookingsRelations = relations(bookings, ({ one, many }) => ({
    ...
    magicToken: one(magicTokens, { // 👈 NUEVO: Relación 1 a 1 con el token
        fields: [bookings.id],
