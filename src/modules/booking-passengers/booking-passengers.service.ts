...
export class BookingPassengerService {
    static async cancelPassenger(
        ...
        return await db.transaction(async (tx) => {
            ...
            if (booking.status === "CANCELLED") throw new Error("La reserva ya está cancelada por completo");
            ...
            if (targetPassenger.isTitular) {
                ...
                throw new Error("No puedes remover al titular de forma parcial. Cancela la reserva completa.");
            }
            ...
            if (penaltyAmount > unitPrice) {
                throw new Error(`La penalización ($${penaltyAmount}) no puede superar el costo individual del asiento ($${unitPrice}).`);
            }
    ...
    static async addOrReactivatePassenger(
        ...
        return await db.transaction(async (tx) => {
            ...
            if (!booking) throw new Error("Reserva no encontrada");
            if (booking.status === "CANCELLED") throw new Error("No se puede modificar una reserva cancelada");
